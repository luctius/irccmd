...
echo '!hudson'" build irccmd ARCH=amd64 DIST=lucid " | irccmd --keepreading --name=buildbot -d --channel=#dev | grep -l "Project irccmd build"
...
echo '!hudson'" build irccmd ARCH=i386 DIST=lucid " | irccmd --keepreading --name=buildbot -d --channel=#dev | grep -l "Project irccmd build"
